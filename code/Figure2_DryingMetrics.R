...
dry_dur = ggplot(data=dat, aes(x = dry_dur,y=Name,group=paste(Name,CLASS),fill=Name,linetype=CLASS)) + 
...
region.mean  = dat.mean %>% group_by(Name,CLASS)%>%
  select(Name,CLASS,mean_dry_date_start,mean_drying_rate,mean_peak2zero,mean_dry_dur) %>%
...
binned = drying_rate + peak2zero + dry_dur
