#####################################################################
##
## Script name: dryingRegimes_timeseriesLULC.R
##
## Author: Samuel Zipper
##        Edited: Adam N. Price
##
...
############################# Description ##########################
##
## Takes LULC datasets and provides a timeseries of LULC for drying events
##   
##
############################# Packages #############################
...
yr_start <- min(clust$meteorologic_year)
yr_stop <- max(clust$meteorologic_year)

## load LULC data
lulc_historic <-   
  readr::read_csv( "data/LULC_historic.csv") %>% 
  dplyr::select(-rownum) %>% 
  dplyr::mutate(gage_ID = as.numeric(gage_ID)) %>% 
  subset(year >= yr_start & year <= yr_stop & 
           gage_ID %in% clust$gage) %>% 
  dplyr::mutate(Source = "Historic")

lulc_hindcast <-   
  readr::read_csv("data/LULC_hindcast.csv") %>% 
  dplyr::select(-rownum) %>% 
  dplyr::mutate(gage_ID = as.numeric(gage_ID)) %>% 
  subset(year >= yr_start & year <= yr_stop & 
           gage_ID %in% clust$gage) %>% 
...
lulc_nlcd <-   
  readr::read_csv("data/LULC_nlcd.csv") %>% 
  dplyr::select(-rownum) %>% 
  dplyr::mutate(gage_ID = as.numeric(gage_ID)) %>% 
  subset(year >= yr_start & year <= yr_stop & 
           gage_ID %in% clust$gage) %>% 
