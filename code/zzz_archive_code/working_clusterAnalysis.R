...
# set.seed(42)
# trainIndex <- createDataPartition(df$gage, p = .05,
#                                   list = FALSE,
#                                   times = 1)
# tt = dat.scale[trainIndex,]
# 
# 
# 
# ideal_ward2 <- NbClust(data = tt, method = "ward.D2")
...
# var$contrib
# 
# cor(df$peak_quantile,df$q50)
################### K-Means ######################
fviz_nbclust(dat.scale, kmeans, method = "silhouette") + theme_classic()

wcke<-eclust(dat.scale, "kmeans", hc_metric="euclidean",k=3)
...
gmm = GMM(dat.scale, 3, dist_mode = "maha_dist", seed_mode = "random_subset", km_iter = 10,
