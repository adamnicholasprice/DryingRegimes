...
data_dir<-"C:\\WorkspaceR\\DryingRegimes\\data\\"
results_dir<-"\\nfs\\njones-data\\Research Projects\\DryRiversRCN\\results\\"
...
sheds<-
  #Get list of watershed shapes
  list.files(paste0(data_dir,"gages_sheds\\")) %>% 
  #Convert to billbe
  as_tibble() %>% 
  #filter to only shape files
  filter(str_detect(value,".shp"))
#Use lapply to download shapefiles
sheds<-lapply(
    X=seq(1, nrow(sheds)), 
    FUN = function(x) st_read(paste0(data_dir, "gages_sheds\\",sheds$value[x]))
  ) 
#Bind rows
sheds<- bind_rows(sheds)
