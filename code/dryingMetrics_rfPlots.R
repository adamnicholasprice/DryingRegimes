...
a.df =  read.csv('data/all_rf_importance.csv') %>% 
  ...
  top_n(10) %>%
  mutate(scaledImp = IncMSE/max(IncMSE))

c1.df = read.csv('data/c1_rf_importance.csv') %>%
  select(predictor,IncMSE)%>% 
  top_n(10) %>%
  mutate(scaledImp = IncMSE/max(IncMSE))


c2.df = read.csv('data/c2_rf_importance.csv') %>% 
  select(predictor,IncMSE)%>% 
  top_n(10) %>%
  mutate(scaledImp = IncMSE/max(IncMSE))

c3.df = read.csv('data/c3_rf_importance.csv') %>% 
  select(predictor,IncMSE)%>% 
  top_n(10) %>%
  mutate(scaledImp = IncMSE/max(IncMSE))


c4.df = read.csv('data/c4_rf_importance.csv') %>% 
  select(predictor,IncMSE)%>% 
  top_n(10) %>%
  mutate(scaledImp = IncMSE/max(IncMSE))
...
c1.df$Category =NA
c1.df$Category[which(c1.df$predictor %in% lulc.pred)] <- "Land use"
c1.df$Category[which(c1.df$predictor %in% clim.pred)] <- "Climate"
c1.df$Category[which(c1.df$predictor %in% phys.pred)] <- "Physiography"

c2.df$Category =NA
c2.df$Category[which(c2.df$predictor %in% lulc.pred)] <- "Land use"
c2.df$Category[which(c2.df$predictor %in% clim.pred)] <- "Climate"
c2.df$Category[which(c2.df$predictor %in% phys.pred)] <- "Physiography"

c3.df$Category =NA
c3.df$Category[which(c3.df$predictor %in% lulc.pred)] <- "Land use"
c3.df$Category[which(c3.df$predictor %in% clim.pred)] <- "Climate"
c3.df$Category[which(c3.df$predictor %in% phys.pred)] <- "Physiography"

c4.df$Category =NA
c4.df$Category[which(c4.df$predictor %in% lulc.pred)] <- "Land use"
c4.df$Category[which(c4.df$predictor %in% clim.pred)] <- "Climate"
c4.df$Category[which(c4.df$predictor %in% phys.pred)] <- "Physiography"
...
pdf("docs//randomForest_VarImp.pdf")
pa
dev.off()
...
pdf("docs//randomForest_VarImp_ALL.pdf")
p.all
dev.off()



p1= ggplot(c1.df,aes(x = reorder(predictor,IncMSE),y = IncMSE,fill=Category)) +
  geom_bar(stat="identity", position="dodge")+
  coord_flip()+
  scale_fill_manual(values=cols) +
  ylab("Variable Importance")+
  xlab(NULL)+
  scale_x_discrete(labels=fullname)+
  theme_minimal()

p2 = ggplot(c2.df,aes(x = reorder(predictor,IncMSE),y = IncMSE,fill=Category)) +
  geom_bar(stat="identity", position="dodge")+
  coord_flip()+
  scale_fill_manual(values=cols) +
  ylab("Variable Importance")+
...
p3 = ggplot(c3.df,aes(x = reorder(predictor,IncMSE),y = IncMSE,fill=Category)) +
  geom_bar(stat="identity", position="dodge")+
  coord_flip()+
  scale_fill_manual(values=cols) +
  ylab("Variable Importance")+
...
p4 = ggplot(c4.df,aes(x = reorder(predictor,IncMSE),y = IncMSE,fill=Category)) +
  geom_bar(stat="identity", position="dodge")+
  coord_flip()+
  scale_fill_manual(values=cols) +
  ylab("Variable Importance")+
