...
metrics = c("peak2zero","drying_rate","dry_dur","peak_quantile","dry_date_start")
form_met = c("Dry-down duration",'Drying Rate','No Flow Duration','Antecedent peak quanle','No flow start date')
eco  = unique(df$AggEcoregion)
D = data.frame(NA)
p = data.frame(NA)
lab = data.frame(NA)
...
for (m in 1:length(metrics)){
  eco_df = df %>%
    select(AggEcoregion,metrics[m])%>%
    set_names(.,c("AggEcoregion","value"))
  for( g1 in groups ) {
    for( g2 in groups ) {
      d.stat[ g1, g2 ] <- ks.test( 
        eco_df$value[eco_df$AggEcoregion == g1], 
        eco_df$value[ eco_df$AggEcoregion == g2 ]
      )$statistic
      p.val[ g1, g2 ] <- ks.test( 
        eco_df$value[eco_df$AggEcoregion == g1], 
        eco_df$value[ eco_df$AggEcoregion == g2 ]
