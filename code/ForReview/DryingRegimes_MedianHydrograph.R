#####################################################################
##
## Script name: DryingRegimes_MedianHydrograph.R
##
## Author: Adam N. Price
##
...
clust = read.csv('data/kmeans_NoFreq.csv')

clust.metrics = clust %>%
  select(gage,peak_date,peak_quantile,peak2zero,drying_rate,dry_date_start,dry_dur)

clust.median = clust %>% 
  select(peak_date,peak_quantile,peak2zero,drying_rate,dry_date_start,dry_dur) %>%
...
median.peak = c(median(clust$dry_date_start),
                median(clust[clust$kmeans==1,'dry_date_start']),
                median(clust[clust$kmeans==2,'dry_date_start']),
                median(clust[clust$kmeans==3,'dry_date_start']),
                median(clust[clust$kmeans==4,'dry_date_start']))
...
cluster.labels <-
  c("11120000" = 'Cluster 1',
    "09505350" = 'Cluster 2',
    "02304500" = 'Cluster 3',
    "11119940" = 'Cluster 4',
    "07315200" = "Drying Event Average")
...
# 
# clust.metrics = clust %>%
#   select(kmeans,peak_date,peak_quantile,peak2zero,drying_rate,dry_date_start,dry_dur)
# 