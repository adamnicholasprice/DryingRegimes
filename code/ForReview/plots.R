...
p = peak2zero + drying_rate + dry_date_start + dry_dur + peakQ + 
  plot_layout(ncol = 2, guides = "collect") & theme(legend.position = "none")

p

pdf("docs/response_plots/boxPlots.pdf")
p
dev.off()
...
coors = left_join(k.means,tt,by=c('gage'='STAID'))

k.means$dec_lat_va = jitter(coors$LAT_GAGE,factor = 1)
k.means$dec_long_va = jitter(coors$LNG_GAGE,factor = 1)

library(leaflet)

yy = st_as_sf(k.means,coords=c("dec_long_va","dec_lat_va"),crs=4326)
library(mapview)

mapview(yy,zcol="cluster_noFreq",label="gage")
